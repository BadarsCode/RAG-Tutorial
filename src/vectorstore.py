...
class FaissVectorStore:
    def __init__(self, persist_dir: str = 'faiss_store', embedding_model: str='all-MiniLM-L6-v2', chunk_size: int=1000, chunk_overlap: int=20):
        ...
        print(f"[INFO] loaded Embedded modle {self.embedding_model}")
    ...
    def add_embeddings(self, embeddings: np.ndarray, metadatas: List[Any]=None):
        ...
        print(f"[INFO] added {embeddings.shape[0]} embeddings to vector store")
    ...
    def search(self, query_embeddings: np.ndarray, top_k: int=10):
        Dist, Ind = self.index.search(query_embeddings, top_k)
        results= []
        for idx, dist in zip(Ind[0], Dist[0]):
            meta = self.metadata[idx] if idx < len(self.metadata) else None
            results.append({"index": idx, "Distence": dist, "metadata": meta})
        return results
    def query(self, query_text: str, top_k: int=5):
        print(f"[INFO] Querying vector store with query text: {query_text}")
